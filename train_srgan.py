...
@tf.function
def train_step(model, img_input, img_target):
  """
  Single step of generator pre-training.
  Args:
    model: A model object with a tf keras compiled generator.
    img_input: The low resolution image tensor.
    img_target: The high resolution image tensor.
  """
  ...
  with tf.GradientTape(persistent=True) as tape:
    ...
    gen_output = model.generator(img_input, training=True)

    # Train discriminators (original images = real ; generated = Fake)
    disc_real_output = model.discriminator(img_target, training=True)
    disc_generated_output = model.discriminator(gen_output, training=True)

    # Calculate Losses
    # gen_loss, adv_loss, mae_loss, mse_loss, content_loss, var_loss = model.generator_loss(disc_generated_output, gen_output, img_target)
    # disc_loss = model.discriminator_loss(disc_real_output, disc_generated_output)

    # Generator Loss
    content_loss = model.content_loss(img_target, gen_output)
    adv_loss = 1e-3 * loss_object(tf.ones_like(disc_generated_output), disc_generated_output)
    mse_loss = tf.keras.losses.MeanSquaredError()(img_target, gen_output)
    mae_loss = tf.reduce_mean(tf.abs(img_target - gen_output))
    var_loss = 1e-5 * tf.reduce_mean(tf.image.total_variation(img_target - gen_output))
    gen_loss = content_loss + adv_loss + 0*mse_loss + mae_loss + 0*var_loss
  ...
  return gen_loss, adv_loss, mae_loss, mse_loss, content_loss, disc_loss, var_loss
...
def train(model, dataset, args, writer):
  ...
  with writer.as_default():
    ...
    for img_input, img_target in dataset:
      ...
      gen_loss, adv_loss, mae_loss, mse_loss, content_loss, disc_loss, var_loss = train_step(model, img_input, img_target)
      model.iterations += 1
...
def main(args):
  ...
  num_images = len(glob.glob(os.path.join(image_dir, "*/*")))
  ...
  for epoch in range(args.epochs):
    ...
    try:
      ...
      train_begin = time()
      gen_loss, adv_loss, mae_loss, mse_loss, content_loss, disc_loss, var_loss = train(model, ds, args, train_summary_writer)
      train_end = time()
...
if __name__ == '__main__':
  ...
  # parser.add_argument('--model_dir', default="./models", type=str, help='Model directory if different from ./models/srgan.h5.')
  ...
  args.image_dir = get_path(args.image_dir)
  args.model_dir = get_path(args.model_dir)
  args.logdir = get_path(args.logdir)
  ...
  for k, v in vars(args).items():
    print(f"  {k}:".ljust(20) + f"{repr(v)}".ljust(70) + f"['{type(v).__name__}']")
